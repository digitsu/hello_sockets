defmodule HelloSocketsWeb.AuthSocketTest do
  ...
  describe "connect/3 error" do
    ...
    test "cannot be connected to without a token" do
      ...
      assert capture_log(fn -> assert :error = connect(AuthSocket, params) end) =~
               "[error] #{AuthSocket} connect error missing params"
    end

    test "cannot be connect to with a nonsense token" do
  ...
  describe "id/1" do
    test "an identifier is based on the connected user ID" do
